...
const AdminProfile = () => {
  ...
  const adminViewBlogs = async () => {
    ...
    let data = await res.json();
    if (data.success) {
      setBlogs(data?.myBlog);
    }
  }
  // commented blog 
  const adminCommentedBlogs = async () => {
    let res = await fetch(`http://localhost:8080/admin/commented_blog`, ({
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${auth?.token?.token}`
    ...
    if (data.success) {
      setCommentedBlogs(data?.allBlog);
    }
  }
  // liked blog 
  const adminLikedBlogs = async () => {
    let res = await fetch(`http://localhost:8080/admin/liked_blog`, ({
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${auth?.token?.token}`
                  ...
                  <div className="card text-center">
                    ...
                    <div className="card-body">
                      ...
                      <div className="d-flex justify-content-evenly">
                        <Link to={'/admin/viewblogs'} className="btn btn-success">
                          <h5 className="card-title">Blogs</h5>
                          <p className="card-text">{blogs.length}</p>
                        </Link>
                        <Link to={'/admin/commented_blog'} className="btn btn-primary">
                          <h5 className="card-title">Commented</h5>
                          <p className="card-text">{commentedBlogs.length}</p>
                        </Link>
                        <Link to={'/admin/liked_blog'} className="btn btn-danger">
